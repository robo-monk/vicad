...
static float choose_grid_step(float target_world) {
    if (!std::isfinite(target_world) || target_world <= 1e-6f) return 1.0f;
    const float exp10 = std::floor(std::log10(target_world));
    const float base = std::pow(10.0f, exp10);
    const float m = target_world / base;
    float snapped = 1.0f;
    if (m <= 1.0f) snapped = 1.0f;
    else if (m <= 2.0f) snapped = 2.0f;
    else if (m <= 5.0f) snapped = 5.0f;
    else snapped = 10.0f;
    return snapped * base;
}

static void draw_grid(const Vec3 &target, float distance, float fov_degrees, i32 viewport_w, i32 viewport_h) {
    const float vh = (float)(viewport_h > 0 ? viewport_h : 1);
    const float vw = (float)(viewport_w > 0 ? viewport_w : 1);
    const float world_per_px = (2.0f * distance * std::tan((fov_degrees * 3.1415926535f / 180.0f) * 0.5f)) / vh;
    const float minor_step = choose_grid_step(world_per_px * 32.0f);
    const float major_step = minor_step * 10.0f;
    const float half_span = std::fmax(vw, vh) * world_per_px * 0.9f;

    const int min_x = (int)std::floor((target.x - half_span) / minor_step);
    const int max_x = (int)std::ceil((target.x + half_span) / minor_step);
    const int min_z = (int)std::floor((target.z - half_span) / minor_step);
    const int max_z = (int)std::ceil((target.z + half_span) / minor_step);
    const float z0 = (float)min_z * minor_step;
    const float z1 = (float)max_z * minor_step;
    const float x0 = (float)min_x * minor_step;
    const float x1 = (float)max_x * minor_step;

    glDisable(GL_LIGHTING);
    glLineWidth(1.0f);
    glBegin(GL_LINES);
    for (int ix = min_x; ix <= max_x; ++ix) {
        const float x = (float)ix * minor_step;
        const bool major = std::fabs(std::fmod(std::fabs(x), major_step)) <= (minor_step * 0.05f) ||
                           std::fabs(major_step - std::fmod(std::fabs(x), major_step)) <= (minor_step * 0.05f);
        if (std::fabs(x) <= minor_step * 0.5f) {
            glColor3f(0.94f, 0.56f, 0.56f);
        } else {
            const float c = major ? 0.80f : 0.86f;
            glColor3f(c, c, c);
        }
    ...
    for (int iz = min_z; iz <= max_z; ++iz) {
        const float z = (float)iz * minor_step;
        const bool major = std::fabs(std::fmod(std::fabs(z), major_step)) <= (minor_step * 0.05f) ||
                           std::fabs(major_step - std::fmod(std::fabs(z), major_step)) <= (minor_step * 0.05f);
        if (std::fabs(z) <= minor_step * 0.5f) {
            glColor3f(0.56f, 0.86f, 0.60f);
        } else {
            const float c = major ? 0.80f : 0.86f;
            glColor3f(c, c, c);
        }
...
static void draw_dimension_arrowheads_world(const Vec3 &a,
    ...
    // Build arrows in a camera-facing plane to keep apparent screen size stable.
    Vec3 dir_screen = project_onto_plane(dir, camera.forward);
    if (dot(dir_screen, dir_screen) <= 1e-8f) {
        dir_screen = project_onto_plane(dir, camera.up);
    }
    if (dot(dir_screen, dir_screen) <= 1e-8f) {
        dir_screen = camera.right;
    }
    dir_screen = normalize(dir_screen);
    Vec3 side = normalize(cross(camera.forward, dir_screen));
    if (dot(side, side) <= 1e-8f) side = camera.up;

    const float arrow_len = clampf(size, 0.0f, len * 0.30f);
    // Equilateral arrowhead: half base = altitude / sqrt(3)
    const float arrow_half_w = arrow_len * 0.57735026919f;
    const Vec3 tail_a = add(a, mul(dir_screen, arrow_len));
    const Vec3 tail_b = add(b, mul(dir_screen, -arrow_len));
...
static void draw_contour_dimensions(const vicad::ScriptSketchContour &contour,
    ...
    if (kind == SketchDimShapeKind::Rectangle || kind == SketchDimShapeKind::Square) {
        ...
        draw_dimension_arrowheads_world(w03, w13, ctx.camera,
                                        arrow_world_size_at_anchor(ctx, w03, w13, mul(add(w03, w13), 0.5f)));
        draw_dimension_arrowheads_world(h03, h13, ctx.camera,
...
static void draw_sketch_dimension_model(const vicad::SketchDimensionModel &model,
    ...
    for (const vicad::SketchDimensionEntity &entity : model.entities) {
        ...
        const Vec3 b3 = add(vec3_from_2d({(float)entity.line.b.x, (float)entity.line.b.y}, z), dim_plane_lift);
        draw_world_line(a3, b3);
        const Vec3 mid = mul(add(a3, b3), 0.5f);
        const float arrow_size = arrow_world_size_at_anchor(ctx, a3, b3, mid);
        draw_dimension_arrowheads_world(a3, b3, ctx.camera, arrow_size);
...
static bool ray_plane_intersection(const Vec3 &ray_origin,
                                   const Vec3 &ray_dir,
...
static std::string normalize_path_lex(const std::string &path) {
...
static void save_recent_files(const std::vector<std::string> &recent_files) {
    std::ofstream out(kRecentFilesPath, std::ios::trunc);
    if (!out) return;
...
static std::vector<std::string> load_recent_files(void) {
    std::vector<std::string> recent;
    std::ifstream in(kRecentFilesPath);
    std::string line;
    while (std::getline(in, line)) {
        std::string path = trim_copy(line);
        if (path.empty()) continue;
        path = normalize_path_lex(path);
        if (!file_exists_path(path)) continue;
        if (std::find(recent.begin(), recent.end(), path) != recent.end()) continue;
        recent.push_back(path);
        if (recent.size() >= kMaxRecentFiles) break;
    }
    return recent;
}

static void push_recent_file(std::vector<std::string> *recent_files, const std::string &path) {
    if (!recent_files) return;
    const std::string norm = normalize_path_lex(path);
    if (norm.empty()) return;
    recent_files->erase(std::remove(recent_files->begin(), recent_files->end(), norm), recent_files->end());
    recent_files->insert(recent_files->begin(), norm);
    if (recent_files->size() > kMaxRecentFiles) {
...
static std::string recent_display_name(const std::string &path) {
    ...
    if (file.size() > suffix_len && file.compare(file.size() - suffix_len, suffix_len, suffix) == 0) {
        file.resize(file.size() - suffix_len);
    }
...
#ifdef VICAD_HAS_NFD
    nfdchar_t *picked = NULL;
    const nfdresult_t result = NFD_OpenDialog("ts", NULL, &picked);
    if (result == NFD_OKAY) {
        if (picked && picked[0] != '\0') {
            if (out_path) *out_path = normalize_path_lex(std::string(picked));
            std::free(picked);
            return true;
        }
        if (picked) std::free(picked);
        if (err) *err = "No file path was returned by file dialog.";
        return false;
    }
    if (result == NFD_CANCEL) return false;
    if (err) *err = std::string("Open dialog failed: ") + (NFD_GetError() ? NFD_GetError() : "unknown error");
    return false;
#else
    if (err) *err = "Open dialog is unavailable (nativefiledialog not compiled).";
    return false;
#endif
}
...
static int AppRunLoop() {
    ...
    vicad_scene::SceneSessionState scene_session = {};
    scene_session.script_path = "myobject.vicad.ts";
    std::vector<std::string> recent_files = load_recent_files();
    if (!recent_files.empty() && file_exists_path(recent_files.front())) {
        scene_session.script_path = recent_files.front();
    } else {
    ...
    while (!RGFW_window_shouldClose(win)) {
        ...
        while (RGFW_window_checkEvent(win, &event)) {
            ...
            if (event.type == RGFW_mouseScroll) {
                const float next_distance = clampf(distance * std::pow(0.9f, event.scroll.y), 0.25f, 2000.0f);
                if (std::fabs(next_distance - distance) > 1e-6f) {
                    i32 mouse_x = 0;
                    i32 mouse_y = 0;
                    RGFW_window_getMouse(win, &mouse_x, &mouse_y);
                    i32 mouse_px_x = 0;
                    i32 mouse_px_y = 0;
                    vicad_picking::WindowMouseToPixel(mouse_x, mouse_y, window_w, window_h, width, height,
                                                      &mouse_px_x, &mouse_px_y);

                    const Vec3 eye_before = camera_position(target, yaw_deg, pitch_deg, distance);
                    const CameraBasis basis_before = camera_basis(eye_before, target);
                    const Vec3 ray_before = camera_ray_direction(mouse_px_x, mouse_px_y, width, height,
                                                                 fov_degrees, basis_before);

                    const Vec3 eye_after = camera_position(target, yaw_deg, pitch_deg, next_distance);
                    const CameraBasis basis_after = camera_basis(eye_after, target);
                    const Vec3 ray_after = camera_ray_direction(mouse_px_x, mouse_px_y, width, height,
                                                                fov_degrees, basis_after);

                    Vec3 hit_before = {0.0f, 0.0f, 0.0f};
                    Vec3 hit_after = {0.0f, 0.0f, 0.0f};
                    const Vec3 zoom_plane_point = target;
                    const Vec3 zoom_plane_normal = basis_before.forward;
                    if (ray_plane_intersection(eye_before, ray_before, zoom_plane_point, zoom_plane_normal, &hit_before) &&
                        ray_plane_intersection(eye_after, ray_after, zoom_plane_point, zoom_plane_normal, &hit_after)) {
                        target = add(target, sub(hit_before, hit_after));
                    }
                }
            ...
            if (event.type == RGFW_mouseButtonPressed && event.button.value == RGFW_mouseLeft) {
                ...
                if (!alt_down) {
                    ...
                    pick_ctx.fov_degrees = fov_degrees;
                    pick_ctx.eye = eye;
...
bool AppInit(AppKernel *app, std::string *error) {
    if (!app) {
        if (error) *error = "AppInit received null app pointer.";
        return false;
    }
    if (app->impl) return true;
    AppKernelImpl *impl = new (std::nothrow) AppKernelImpl();
    if (!impl) {
        if (error) *error = "Failed to allocate app kernel state.";
        return false;
    }
    app->impl = impl;
    return true;
...
bool AppTick(AppKernel *app, std::string *error) {
    if (!app || !app->impl) {
        if (error) *error = "AppTick called before AppInit.";
        return false;
    }
    AppKernelImpl *impl = (AppKernelImpl *)app->impl;
    if (impl->has_run) return false;
    impl->has_run = true;
    impl->exit_code = AppRunLoop();
    if (impl->exit_code != 0 && error && error->empty()) {
        *error = "Application loop exited with a non-zero status.";
    }
    return false;
}
